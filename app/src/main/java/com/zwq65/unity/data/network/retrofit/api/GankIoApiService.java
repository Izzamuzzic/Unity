...
public interface GankIoApiService {
     ...
     * 获取福利图片
     */
    @GET(ApiConstants.Welfare + ApiConstants.defaultPageSize + "/{page}")
    Flowable<GankApiResponse<List<Image>>> getImages(@Path("page") int page);

    /**
     * 获取福利图片（20张）
     */
    @GET(ApiConstants.Welfare + ApiConstants.morePageSize20 + "/{page}")
    Flowable<GankApiResponse<List<Image>>> get20Images(@Path("page") int page);

    /**
     * 获取休息视频
     */
    @GET(ApiConstants.RestVideo + ApiConstants.defaultPageSize + "/{page}")
    Flowable<GankApiResponse<List<Video>>> getVideos(@Path("page") int page);

    /**
     * 获取android文章
     */
    @GET(ApiConstants.Android + ApiConstants.defaultPageSize + "/{page}")
    Flowable<GankApiResponse<List<Article>>> getAndroidArticles(@Path("page") int page);

    /**
     * 获取ios文章
     */
    @GET(ApiConstants.IOS + ApiConstants.defaultPageSize + "/{page}")
    Flowable<GankApiResponse<List<Article>>> getIosArticles(@Path("page") int page);

    /**
     * 获取前端文章
     */
    @GET(ApiConstants.Qianduan + ApiConstants.defaultPageSize + "/{page}")
